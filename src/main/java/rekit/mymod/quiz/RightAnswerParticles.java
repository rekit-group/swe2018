...
@LoadMe
public class RightAnswerParticles extends DynamicInanimate {
	
	ParticleSpawner ps;
	long sumTime = 2000; // to make it spawn once when created
	
	private RightAnswerParticles() {
		super();
	}
	
	public RightAnswerParticles(Vec startPos) {
	...
	public DynamicInanimate create(Vec startPos, String... options) {
		return new RightAnswerParticles(startPos);
	}
